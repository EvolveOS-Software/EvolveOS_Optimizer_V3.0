    ...
    public sealed partial class ContextMenuEditorWindow : Window
    {
        ...
        private void LoadQuickTemplates()
        {
            _quickTemplates = new List<ContextMenuTemplate>
            {
                ...
                new ContextMenuTemplate { Title = ResourceString.GetString("cme_qt_reboot_bios_title") ?? "Reboot to BIOS / UEFI", Description = ResourceString.GetString("cme_qt_reboot_bios_desc") ?? "Restarts the PC directly into the motherboard firmware settings", ExePath = "powershell.exe", Arguments = "-WindowStyle Hidden -Command Start-Process shutdown -ArgumentList '/r /fw /f /t 0' -Verb RunAs", TargetIndex = 2, RunAsAdmin = true, HiddenWindow = true }
                #endregion
            };

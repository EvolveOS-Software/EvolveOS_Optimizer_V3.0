        ...
        internal async Task ApplyTweaks(string tweak, bool isDisabled, CancellationToken token = default)
        {
            ...
            switch (tweak)
            {
                ...
                case "TglButton8":
                    ...
                    if (isDisabled)
                    {
                        await CommandExecutor.RunCommandAsTrustedInstaller(@$"/c net stop spooler && del /F /Q %systemroot%\System32\spool\PRINTERS\*.*");
                    }
                ...
                case "TglButton30":
                    ...
                    await SetTaskState(!isDisabled, token, retailTasks);
                    break;
            }
        }

...
{
    internal sealed class PrivacyTweaks : FirewallManager
    {
        ...
        internal bool IsDefaultHosts()
        {
            try
            {
                if (!File.Exists(PathLocator.Files.Hosts)) return true;
            ...
            switch (tweak)
            {
                ...
                case "TglButton9":
                    ...
                    await Task.Run(async () =>
                    {
                        ...
                        string hostsPath = PathLocator.Files.Hosts;
                        string backupFile = hostsPath + " (Default EvolveOS_Optimizer)";
                        ...
                        try
                        {
                            if (isDisabled)
                            {
                                ...
                                {
                                    await CommandExecutor.RunCommandAsTrustedInstaller($"/c copy /y \"{hostsPath}\" \"{backupFile}\"");
                                }

                                byte[] resourceData = ArchiveManager.GetResourceBytes("hosts.gz");
                                if (resourceData.Length > 0) ArchiveManager.Unarchive(hostsPath, resourceData);
                            }
                            else
                            {
                                if (File.Exists(backupFile))
                                {
                                    await CommandExecutor.RunCommandAsTrustedInstaller($"/c move /y \"{backupFile}\" \"{hostsPath}\"");
                                }
                                else
                                {
                                    await CommandExecutor.RunCommandAsTrustedInstaller($"/c type Array > \"{hostsPath}\"");
                                }
                            }

                            await LocalNetworkFlushAsync();
                        }
                        catch (Exception ex)
                        {
                ...
                case "TglButton15":
                    ...
                    else
                    {
                        ...
                        RegistryHelp.Write(Registry.LocalMachine, $@"{diagsvc}\TriggerInfo\0", "Data0", Array.ConvertAll("46,00,44,00,44,00,34,00,35,00,39,00,32,00,34,00,2d,00,37,00,38,00,34,00,41,00,2d,00,34,00,46,00,38,00,39,00,2d,00,42,00,44,00,38,00,44,00,2d,00,32,00,46,00,34,00,32,00,42,00,42,00,42,00,45,00,39,00,39,00,36,00,45,00,00,00".Split(','), s => Convert.ToByte(s, 16)), RegistryValueKind.Binary);
                    ...
                    break;
                case "TglButton16":
                    await CommandExecutor.RunCommandAsTrustedInstaller($@"/c reg add HKLM\SYSTEM\CurrentControlSet\Services\diagnosticshub.standardcollector.service /t REG_DWORD /v Start /d {(isDisabled ? "4" : "3")} /f");
        ...
        #region Native Network Cache P/Invokes

        [DllImport("dnsapi.dll", EntryPoint = "DnsFlushResolverCache")]
        private static extern int DnsFlushResolverCache();

        [DllImport("Iphlpapi.dll", SetLastError = true)]
        private static extern uint FlushIpNetTable(int dwIfIndex);

        private async Task LocalNetworkFlushAsync()
        {
            await Task.Run(() =>
            {
                try
                {
                    int dnsResult = DnsFlushResolverCache();
                    if (dnsResult != 0) Debug.WriteLine($"[DNS] Native flush error: {dnsResult}");

                    uint arpResult = FlushIpNetTable(0);
                    if (arpResult != 0) Debug.WriteLine($"[ARP] Native flush error code: {arpResult}");
                }
                catch (Exception ex)
                {
                    ErrorLogging.LogDebug(ex);

                    _ = CommandExecutor.RunCommand("/c ipconfig /flushdns & netsh interface ip delete arpcache");
                }
            });
        }

...
namespace EvolveOS_Optimizer.Utilities.Tweaks
{
    internal sealed class UninstallingPakages : TaskSchedulerManager
    {
        ...
        internal static Dictionary<string, (string? Alias, bool IsUnavailable, List<string>? Scripts)> PackagesDetails = new Dictionary<string, (string?, bool, List<string>?)>()
        {
        ...
        internal static async void CheckingForLocalAccount()
        {
        ...
        internal static SystemTask RestoreOneDriveFolder()
        {
            return SystemTask.Run(async () =>
            {
                await CommandExecutor.InvokeRunCommand($@"/c {PathLocator.Executable.OneDrive}").ConfigureAwait(false);

                await SetTaskState(true, TaskStorage.oneDriveTask);
        ...
        internal static SystemTask RemoveAppxPackage(string packageName, bool shouldRemoveWebView = false)
        {
            if (packageName == "OneDrive")
            {
                return SystemTask.Run(async () =>
                {
                    ...
                    await SetTaskState(false, TaskStorage.oneDriveTask);

                    await CommandExecutor.RunCommand($@"/c rd /s /q %userprofile%\AppData\Local\Microsoft\OneDrive & rd /s /q %userprofile%\AppData\Local\OneDrive & rd /s /q ""%allusersprofile%\Microsoft OneDrive"" & rd /s /q {PathLocator.Folders.SystemDrive}OneDriveTemp{(_isLocalAccount ? @" & rd /s /q %userprofile%\OneDrive" : "")}");
                });
            }
            ...
            return SystemTask.Run(async () =>
            {
                ...
                {
                    case "Maps":
                        await RemoveTasks(TaskStorage.mapsTasks);
                        break;
                    case "Widgets":
                    ...
                    case "Phone":
                        ...
                        await CommandExecutor.RunCommandAsTrustedInstaller($@"/c reg delete ""HKEY_CLASSES_ROOT\CLSID\{{7AD84985-87B4-4a16-BE58-8B72A5B390F7}}"" /f & reg delete ""HKEY_CLASSES_ROOT\Wow6432Node\CLSID\{{7AD84985-87B4-4a16-BE58-8B72A5B390F7}}"" /f");
                        break;
                    case "Paint3D":
                        try
                        {
                        ...
                        break;
                    case "Edge":
                        string[] processes = { "msedge", "edgeupdate", "edgeupdatem", "msedgewebview2", "microsoftedgeupdate", "msedgewebviewhost", "msedgeuserbroker", "usocoreworker", "widgets", "microsoftedgesh", "microsoftedgecp", "microsoftedge" };
                        await CommandExecutor.RunCommandAsTrustedInstaller("/c taskkill /f " + string.Join(" ", processes.Select(p => $"/im {p}.exe")));

                        await RemoveTasks(TaskStorage.edgeTasks);

                        await CommandExecutor.RunCommandAsTrustedInstaller(CommandExecutor.CleanCommand(string.Join(" & ", new[] {
                        @"/c rmdir /s /q ""%LocalAppData%\Microsoft\Edge""",
                        @"for /r ""%AppData%\Microsoft\Internet Explorer\Quick Launch"" %f in (*Edge*) do del ""%f""",
                        @"del /q /f ""%AppData%\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\*Edge*.lnk""",
                        $@"for /r ""{PathLocator.Folders.SystemDrive}ProgramData\Microsoft\Windows\Start Menu\Programs"" %f in (*Edge*) do del ""%f""",
                        $@"for /r ""{PathLocator.Folders.SystemDrive}Users"" %f in (*Edge*) do @if exist ""%f"" del /f /q ""%f""",
                        $@"for /d %d in (""{PathLocator.Folders.SystemDrive}Program Files (x86)\Microsoft\*Edge*"") do rmdir /s /q ""%d""",
                        ...
                        async SystemTask RemoveDirectory(string path)
                        {
                        ...
                        try
                        {
                            ...
                            if (key != null)
                            {
                                foreach (string subKey in key.GetSubKeyNames())
                                {
                                    ...
                                    string? pathValue = subKeyEntry?.GetValue("Path") as string;
                                    if (!string.IsNullOrEmpty(pathValue) && pathValue.Contains("Edge", StringComparison.OrdinalIgnoreCase))
                                    {
                                        if (!shouldRemoveWebView && pathValue.Contains("WebView"))
                                        {
                                            continue;
                                        }

                                        if (pathValue.EndsWith(@"\AppxManifest.xml", StringComparison.OrdinalIgnoreCase))
                                        {
                                            pathValue = pathValue.Replace(@"\AppxManifest.xml", "").Trim();
                                        }

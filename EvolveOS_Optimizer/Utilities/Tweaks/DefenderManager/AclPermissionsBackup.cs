...
namespace EvolveOS_Optimizer.Utilities.Tweaks.DefenderManager
{
    internal class AclPermissionsBackup : TaskSchedulerManager
    {
        ...
        {
            if (!File.Exists(PathLocator.Files.BackupJsonWD) || !File.Exists(PathLocator.Files.BackupAclWD)) return;

            try
            {
                var allValues = JsonConvert.DeserializeObject<Dictionary<string, Dictionary<string, RegistryValueBackup>>>(File.ReadAllText(PathLocator.Files.BackupJsonWD));
                var aclDataDict = JsonConvert.DeserializeObject<Dictionary<string, string>>(File.ReadAllText(PathLocator.Files.BackupAclWD));
                ...
                foreach (var kv in allValues)
                {
                    ...
                    if (!Enum.TryParse(parts[0], out RegistryView view)) continue;
        ...
        private static object ConvertBackupValueToClr(object rawValue, RegistryValueKind kind)
        {
            ...
            {
                case RegistryValueKind.Binary:
                    if (rawValue is string s) return Convert.FromBase64String(s);
                ...
                case RegistryValueKind.MultiString:
                    if (rawValue is string[] sa) return sa;
                    if (rawValue is IEnumerable<object> ie) return ie.Select(x => x.ToString() ?? string.Empty).ToArray();
        ...
        private static object ConvertJTokenToObject(object tokenOrObj)
        {
            ...
            {
                return t.ToObject<object>() ?? new object();
            }
            return tokenOrObj;
        }
    }

...
public class PowerShellRunner : IPowerShellRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"evolveos_{Guid.NewGuid()}.ps1");
        try
        {
    ...
    public async Task ValidateScriptSyntaxAsync(
        ...
        CancellationToken ct = default)
    {
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"evolveos_validate_{Guid.NewGuid():N}.ps1");
        try
        {
...
exit 0";
    ...
    public async Task ValidateXmlSyntaxAsync(
        ...
        CancellationToken ct = default)
    {
        var tempFile = _fileSystemService.CombinePath(_fileSystemService.GetTempPath(), $"evolveos_validate_{Guid.NewGuid():N}.xml");
        try
        {
...
try {
    ...
    private void ReportLine(string line, IProgress<TaskProgressDetail>? progress)
    {

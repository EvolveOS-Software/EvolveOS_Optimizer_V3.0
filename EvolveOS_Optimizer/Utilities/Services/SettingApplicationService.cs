    ...
    ISettingOperationExecutor operationExecutor) : ISettingApplicationService
{
    ...
    public async Task<OperationResult> ApplySettingAsync(ApplySettingRequest request)
    {
        ...
        if (setting.InputType == InputType.NumericRange && value != null)
        {
            ...
            value = intValue.ToString(System.Globalization.CultureInfo.InvariantCulture);
        }
        ...
        if (operationResult.Success && setting.InputType == InputType.NumericRange && !string.IsNullOrEmpty(setting.ActionCommand))
        {
            var commandProvider = actionCommandRegistry.TryGet(settingId);
            if (commandProvider != null)
            {
                logService.Log(LogLevel.Info, $"[SettingApplicationService] Triggering live ActionCommand '{setting.ActionCommand}' for slider '{settingId}'");
                await ExecuteActionCommand(commandProvider, setting, setting.ActionCommand, false, settingId).ConfigureAwait(false);
            }
            else
            {
                logService.Log(LogLevel.Warning, $"[SettingApplicationService] Slider '{settingId}' has ActionCommand '{setting.ActionCommand}', but no provider is registered for this setting ID.");
            }
        }

...
public class ScheduledTaskService(ILogService logService, IFileSystemService fileSystemService) : IScheduledTaskService
{
    ...
    public async Task<OperationResult> UnregisterScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                folder = GetEvolveOSFolder(taskService);
                ...
                {
                    logService.Log(LogLevel.Debug, $"[ScheduledTaskService] Task '{taskName}' not found: {ex.Message}");
                }
    ...
    public async Task<bool> IsTaskRegisteredAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            {
                logService.Log(LogLevel.Debug, $"[ScheduledTaskService] Task '{taskName}' not registered: {ex.Message}");
                return false;
            }
            finally
            {
    ...
    public async Task<OperationResult> RunScheduledTaskAsync(string taskName)
    {
        return await Task.Run(() =>
        {
            ...
            try
            {
                ...
                if (folder == null)
                {
                    logService.LogError($"EvolveOS Optimizer task folder not found when trying to run: {taskName}");
                    return OperationResult.Failed("EvolveOS Optimizer task folder not found");
                }
    ...
    private async Task<OperationResult> RegisterTaskInternal(string taskName, string? scriptPath, string? username, TaskTriggerType triggerType, string? command = null)
    {
        ...
        {
            taskService = CreateTaskService();
            folder = GetOrCreateEvolveOSFolder(taskService);
    ...
    private static void ReleaseComObject(object? comObject)
    {
        if (comObject != null)
        {
            try { Marshal.ReleaseComObject(comObject); } catch { /* COM release */ }
        }
    }

    private dynamic GetOrCreateEvolveOSFolder(dynamic taskService)
    {
        dynamic rootFolder = taskService.GetFolder("\\");
        try
        {
            return rootFolder.GetFolder("EvolveOS");
        }
        catch (Exception ex)
        {
            logService.Log(LogLevel.Debug, $"[ScheduledTaskService] EvolveOS Optimizer folder doesn't exist, creating: {ex.Message}");
            return rootFolder.CreateFolder("EvolveOS");
        }
        finally
        {
    ...
    private dynamic? GetEvolveOSFolder(dynamic taskService)
    {
        ...
        {
            logService.Log(LogLevel.Debug, $"[ScheduledTaskService] EvolveOS Optimizer folder not found: {ex.Message}");
            return null;
        }
        finally
        {
    ...
    private async Task RemoveExistingTask(dynamic folder, string taskName)
    {
        ...
        {
            logService.Log(LogLevel.Debug, $"[ScheduledTaskService] No existing task '{taskName}' to remove: {ex.Message}");
        }
        finally
        {
            ReleaseComObject(existingTask);

...
/// <summary>
/// Responsible for querying the system to discover the current state of various configuration settings.
/// </summary>
/// <remarks>
/// <para>
/// <b>Responsibility:</b>
/// This service handles the heavy lifting of reading the host's actual configuration. It fetches 
/// raw values from the Windows Registry, Scheduled Tasks, Power Plans, DNS settings, and other 
/// system components, and translates those raw values into standardized <see cref="SettingStateResult"/> objects.
/// </para>
/// <para>
/// <b>Performance:</b>
/// It utilizes batching (where applicable, such as bulk registry reads) to minimize the overhead of querying
/// hundreds of individual settings during UI navigation or application initialization.
/// </para>
/// </remarks>
    ...
    ISystemRestoreService systemRestoreService) : ISystemSettingsDiscoveryService
{
    ...
    private static bool ValuesAreEqual(object? value1, object? value2)
        => ValueComparer.ValuesAreEqual(value1, value2);
    ...
    private int DetectSystemTrayIndex(SettingDefinition setting)
    {
        ...
        if (subKeys.Length == 0)
            return IndexOfScriptOption(setting, ScriptOption.None);
        ...
        if (total == 0)
            return IndexOfScriptOption(setting, ScriptOption.None);
        if (promoted == total)
            return IndexOfScriptOption(setting, ScriptOption.Enabled);
        if (promoted == 0)
            return IndexOfScriptOption(setting, ScriptOption.Disabled);
